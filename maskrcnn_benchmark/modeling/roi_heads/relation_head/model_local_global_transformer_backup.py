...
class RoIFeaturePooler(nn.Module):
    def __init__(self, config, in_dim, out_dim):
        ...
        pooler_hid_dim = 4096
        self.token_linear_pooling = nn.Linear(in_dim, token_pooling_dim)
        self.token_linear_out = self.token_linear_out = nn.Sequential(*[
            nn.Linear(token_pooling_dim * max_length, pooler_hid_dim),
            nn.Linear(pooler_hid_dim, out_dim),
            nn.ReLU()
        ])

    def forward(self, roi_feature):
        obj_cnt = roi_feature.size(0)
        return self.token_linear_out(self.token_linear_pooling(roi_feature).view(obj_cnt, -1))
...
class ObjectLocalEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.obj_embedding = nn.Embedding(num_obj_cls, in_channel)
        ...
        if in_channel != encoder_dim:
            self.linear_in = nn.Linear(in_channel, encoder_dim)
    ...
    def forward(self, roi_features, proposals):
        ...
        if self.linear_out:
            roi_features = self.linear_out(roi_features)
